import express from "express";
const router = express.Router();
import Cal from "../models/CalModel.js";

//add   and read

//post method
router.route("/").post((req, res) => {
  const customer_Id = req.body.customer_Id;

  const total_count = Number(req.body.total_count);

  const fee_based_Activities = Number(req.body.fee_based_Activities);

  const plate_type = Number(req.body.plate_type);
  const additional_charges = Number(req.body.additional_charges);

  const totalSalary = Number(
    total_count * plate_type + fee_based_Activities + additional_charges
  );

  const newCal = new Cal({
    customer_Id,
    total_count,
    fee_based_Activities,
    plate_type,
    additional_charges,
    totalSalary,
  });

  //validation part
  newCal
    .save()
    .then(() => {
      res.json("Calculation Added");
    })
    .catch((err) => {
      console.log(err);
    });
});

//get/read  method

router.route("/").get((req, res) => {
  Cal.find()
    .then((cal) => {
      res.json(cal);
    })
    .catch((err) => {
      console.log(err);
    });
});

// delete and update
//put method
//async task crash wenne nha multiple task orderely wenva
router.route("/:id").put(async (req, res) => {
  //id eka fetch karana widiya
  let userId = req.params.id;

  // D strucher
  const {
    customer_Id,
    total_count,
    fee_based_Activities,
    plate_type,
    additional_charges,
    totalSalary,
  } = req.body;
  const updateCal = {
    customer_Id,
    total_count,
    fee_based_Activities,
    plate_type,
    additional_charges,
    totalSalary,
  };

  //me userge id eka yatathe mehema user kenek innvada kiyala check karanva
  //email,NIC find kranvanm findbyone use karanva
  //objectid find krnvanm  findbyid
  //await eken kiyanne poddak inna complete wenkn

  const update = await Cal.findByIdAndUpdate(userId, updateCal)
    .then(() => {
      res.status(200).send({ status: " User update" });
    })
    .catch((err) => {
      console.log(err);
      res
        .status(500)
        .send({ status: "Error with updating data", error: err.message });
    });
});

router.route("/:id").delete(async (req, res) => {
  let userID = req.params.id;

  await Cal.findByIdAndDelete(userID)
    .then(() => {
      res.status(200).send({ status: "User Deleted" });
    })
    .catch((err) => {
      console.log(err.message);
      res
        .status(500)
        .send({ status: "Erro with delete user", error: err.message });
    });
});

router.route("/:id").get(async (req, res) => {
  let userId = req.params.id;
  const user = await Cal.findById(userId)
    .then((event) => {
      res.status(200).send({ status: "User fetched", event });
    })
    .catch(() => {
      console.log(err.message);
      res
        .status(500)
        .send({ status: "Error withe get user", error: err.message });
    });
});

//module.exports = router; // export karanna one
export default router;
